...
def run_backtest_core(candles, pivots, risk_pct, rr, fib_level, max_bars, max_hold, recency_bars, one_per_pair, min_swing_pct=0.002, stop_buffer_pct=0.001, k_stale=0, entry_mode="rejection", engine="structure"):
    ...
    def run_original():
        """
        Original v6 logic:
        - Scan pivot triplets P1→P2→P3
        - Bear: P1(H)→P2(L)→P3(H) where P3 < P1 (lower high)
        - Bull: P1(L)→P2(H)→P3(L) where P3 > P1 (higher low)
        - Fib drawn from P1→P2 range
        - Entry at 61.8% retracement after P3 confirms
        - SL = P1 + 2% buffer, TP = entry ± rpp * rr
        - Bias flip on SL hit
        """
        nonlocal equity
        orig_trades = []
        ...
        for pi in range(2, len(pivots)):
            ...
            rpp    = abs(fib618 - sl_lvl)
            if rpp <= 0: continue
            tp     = fib618 - rpp * rr    if st=="bear" else fib618 + rpp * rr
            pos    = (equity * risk_pct) / rpp

            # Find entry — touch fib within max_bars, invalidate if P1 broken
            ec = None
            for ci in range(p3["idx"]+1, min(p3["idx"]+max_bars, n)):
                if st == "bear":
                    if highs_[ci] > fh: break        # structure invalidated
                    if highs_[ci] >= fib618: ec=ci; break
                else:
                    if lows_[ci] < fl: break         # structure invalidated
            ...
            pnl    = (fib618 - xp) * pos if st=="bear" else (xp - fib618) * pos
            equity += pnl
            won     = xr == "TP"
...
def live_journal():
    try:
        acc_res = httpx.get(
            f"{SUPABASE_URL}/rest/v1/live_account?id=eq.1&select=*",
            headers=HEADERS, timeout=10
        )
        trades_res = httpx.get(
            f"{SUPABASE_URL}/rest/v1/live_trades?select=*&order=created_at.desc&limit=500",
            headers=HEADERS, timeout=10
        )
        account = acc_res.json()[0] if acc_res.status_code==200 and acc_res.json() else None
        trades  = trades_res.json() if trades_res.status_code==200 else []
