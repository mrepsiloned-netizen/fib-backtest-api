# ============================================================
# FIB PAPER TRADER v4 — Full Trade Journal
# - Entry/exit notifications with full account details
# - P&L tracking in Supabase
...
def init_account():
    ...
    try:
        ...
        res = httpx.post(
            f"{SUPABASE_URL}/rest/v1/paper_account",
...
def update_account(balance, won, pnl):
    ...
    try:
        acc = get_account()
        if not acc:
            acc = {"balance": START_BALANCE, "total_trades": 0, "wins": 0, "losses": 0, "total_pnl": 0.0, "peak_balance": START_BALANCE, "max_drawdown": 0.0}

        new_balance   = round(balance, 4)
        peak          = max(acc["peak_balance"], new_balance)
        drawdown      = round((peak - new_balance) / peak * 100, 2)
        max_dd        = max(acc["max_drawdown"], drawdown)
        ...
        httpx.patch(
            f"{SUPABASE_URL}/rest/v1/paper_account?id=eq.1",
...
def log_trade(trade_data):
    ...
    try:
        httpx.post(
            ...
            json=trade_data, headers={**SUPABASE_HEADERS, "Prefer":"return=minimal"},
            timeout=10
        )
    except Exception as e:
...
<b>If TP hit:</b> Balance → ${tp_gain:.2f} (+${risk_amt*signal["rr"]:.2f})
...
<b>P&L:</b>    {'+' if pnl>=0 else ''}${pnl:.4f} ({'+' if pnl>=0 else ''}{pnl/acc["balance"]*100:.2f}%)
<b>RR:</b>     {signal["rr"]}R {'achieved ✅' if won else 'missed ❌'}

<b>Previous balance:</b> ${acc["balance"]-pnl:.2f}
    ...
    try:
        ...
        if open_signals:
            ...
            for key, sig in open_signals.items():
                ...
                open_str += f"\n• {sym} {tf.upper()} {sig['direction']} @ ${sig['entry']}"
...
def run():
    global open_signals
    print("🤖 Fib Paper Trader v4 starting...")
    ...
    send_telegram(f"""🤖 <b>Fib Paper Trader v4 LIVE</b>
...
📊 Paper trading — full trade journal active""")
    ...
    while True:
        try:
            ...
            # Daily summary at 8AM UTC
            if now_utc.hour == 8 and now_utc.minute < 1:
                        ...
                        if now - last_signal.get(key, 0) > interval:
                            ...
                            open_signals[key] = {
                                **signal,
                                ...
                                "label":label,"entry_time":now
                            }
                            print(f"[{now_str}] ✅ ENTRY: {symbol} {timeframe} {signal['direction']}")
                    else:
            ...
            for key, sig in open_signals.items():
                try:
                    ...
                    if direction=="LONG":
                        if price <= sig["sl"]:
                            won=False; hit=True; exit_price=sig["sl"]
                        elif price >= sig["tp"]:
                            won=True; hit=True; exit_price=sig["tp"]
                    else:
                        if price >= sig["sl"]:
                            won=False; hit=True; exit_price=sig["sl"]
                        elif price <= sig["tp"]:
                    ...
                    if hit:
                        ...
                        pnl      = (sig["entry"]-exit_price)*pos_size if direction=="SHORT" else (exit_price-sig["entry"])*pos_size
                        pnl      = round(pnl, 4)
